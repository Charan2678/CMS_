...
<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
    <div>
        <h1 style="font-size: 1.5rem; font-weight: 800; color: var(--text-primary); margin: 0; display: flex; align-items: center; gap: 0.625rem; letter-spacing: -0.015em;">
            <?= icon('calendar', 'icon-md') ?> Academic Years Management
        </h1>
    ...
    <div style="display: flex; gap: 0.75rem;">
        <span class="badge badge-success" style="font-size: 0.8125rem; padding: 0.45rem 0.9rem;"><?= icon('check-circle-2', 'icon-xs') ?> Active Session: 2026-2027</span>
    </div>
</div>
...
<div class="page-split">
    ...
    <div class="card" style="height: fit-content;">
        <h2 style="font-size: 1.125rem; font-weight: 800; color: var(--text-primary); margin-top: 0; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
            <?= icon('plus', 'icon-sm') ?> Add Academic Year
        </h2>
        ...
        <form method="POST" action="/master/academic-years">
            ...
            <div class="form-group" style="display: flex; align-items: center; gap: 0.625rem; margin-bottom: 1.5rem; background: var(--bg-main); padding: 0.75rem 1rem; border-radius: 10px; border: 1px solid var(--border-color); box-shadow: var(--shadow-xs);">
            ...
            <button type="submit" class="btn-primary" style="width: 100%; padding: 0.875rem 1.5rem;"><?= icon('plus', 'icon-sm') ?> Create Academic Year</button>
        </form>
    </div>
    ...
    <div class="card">
        <h2 style="font-size: 1.125rem; font-weight: 800; color: var(--text-primary); margin-top: 0; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
            <?= icon('clipboard-list', 'icon-sm') ?> Configured Academic Sessions Directory
        </h2>
                        ...
                        <?php foreach ($academicYears as $ay): ?>
                            <tr>
                                ...
                                <td style="color: var(--text-secondary); font-size: 0.8125rem; font-weight: 500;">
                                    <?= icon('calendar', 'icon-xs') ?> <?= date('d M Y', strtotime($ay['start_date'])) ?> &mdash; <?= date('d M Y', strtotime($ay['end_date'])) ?>
                                </td>
                                <td>
                                    <?php if ((int)$ay['is_current'] === 1): ?>
                                        <span class="badge badge-success"><?= icon('check-circle-2', 'icon-xs') ?> Active Session</span>
                                    <?php else: ?>
                                        <span class="badge badge-muted"><?= icon('archive', 'icon-xs') ?> Archived</span>
                                    <?php endif; ?>
                                </td>
                                <td style="text-align: right;">
                                    <?php if ((int)$ay['is_current'] !== 1): ?>
                                        <form method="POST" action="/master/academic-years" style="display: inline;">
                                            ...
                                            <button type="submit" class="btn-primary" style="padding: 0.4rem 1rem; font-size: 0.75rem; border-radius: 8px;"><?= icon('check', 'icon-xs') ?> Set Active</button>
                                        </form>
                                    <?php else: ?>
                                        <span style="font-size: 0.8125rem; color: var(--success); font-weight: 800; display: inline-flex; align-items: center; gap: 0.35rem;">
                                            <?= icon('check-circle-2', 'icon-xs') ?> Current Active
                                        </span>
                                    <?php endif; ?>
                                </td>
                            </tr>
