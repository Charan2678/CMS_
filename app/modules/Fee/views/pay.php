<div style="max-width: 900px; margin: 0 auto;">
    <div style="margin-bottom: 1.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
        <div>
            <h2 style="font-size: 1.35rem; font-weight: 800; color: var(--text-primary); margin: 0; display: flex; align-items: center; gap: 0.5rem; letter-spacing: -0.015em;">
                <?= icon('credit-card', 'icon-md') ?> Fee Checkout &amp; Secure Payment
            </h2>
        ...
        <a href="/fee/payments" class="btn btn-secondary" style="font-size: 0.8125rem; display: inline-flex; align-items: center; gap: 0.35rem;">
            <?= icon('arrow-left', 'icon-xs') ?> Back to Dues Ledger
        </a>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1.2fr 1.8fr; gap: 1.5rem;">
        ...
        <div class="card" style="border-top: 4px solid <?= $upiDetails['badge_color'] ?>; height: fit-content;">
            ...
            <h3 style="font-size: 1.15rem; font-weight: 800; color: var(--text-primary); margin: 0.35rem 0 1rem 0;">
                <?= e($fee['category_name']) ?>
            </h3>

            <div style="background: var(--bg-main); padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color); margin-bottom: 1.25rem; box-shadow: var(--shadow-xs);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.8125rem;">
                    ...
                    <strong style="color: var(--text-primary);"><?= e($fee['first_name'] . ' ' . $fee['last_name']) ?></strong>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.8125rem;">
                    <span style="color: var(--text-secondary);">Roll Number:</span>
                    <strong style="color: var(--accent-color);"><?= e($fee['roll_number']) ?></strong>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.8125rem;">
                ...
                <div style="display: flex; justify-content: space-between; font-size: 0.8125rem; margin-top: 0.35rem;">
                    ...
                    <span style="color: var(--success); font-weight: 700;">₹<?= number_format((float)$fee['total_paid'], 2) ?></span>
                </div>
            </div>

            <div style="background: rgba(2, 132, 199, 0.08); border: 1px solid rgba(2, 132, 199, 0.3); border-radius: 10px; padding: 1rem; text-align: center; box-shadow: var(--shadow-xs);">
        ...
        <!-- Right: Official UPI QR Payment Section -->
        <div class="card" style="border-top: 4px solid var(--accent-color);">
            <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem; margin-bottom: 1.25rem;">
                <h3 style="margin: 0; font-size: 1.05rem; font-weight: 800; color: var(--text-primary); display: flex; align-items: center; gap: 0.5rem;">
                    <?= icon('smartphone', 'icon-sm') ?> Official UPI QR Scan &amp; Pay
                </h3>
            </div>
            ...
            <div id="payTabUpi">
                <div style="text-align: center; margin-bottom: 1.25rem;">
                    <div style="display: inline-block; background: #ffffff; padding: 1.25rem; border-radius: 16px; border: 2px solid var(--border-color); box-shadow: var(--shadow-lg); max-width: 270px; width: 100%;">
                        <div style="display: flex; align-items: center; justify-content: center; gap: 0.35rem; margin-bottom: 0.65rem; color: #0284c7; font-weight: 800; font-size: 0.8125rem;">
                            <?= icon('landmark', 'icon-xs') ?> Official Institutional QR
                        </div>
                        ...
                        <div style="margin-top: 0.75rem;">
                            <a href="<?= e($upiUri) ?>" class="btn btn-sm btn-outline-primary" style="display: inline-flex; align-items: center; gap: 0.35rem; font-size: 0.75rem; font-weight: 700; text-decoration: none; padding: 0.35rem 0.85rem; border-radius: 20px; border: 1px solid var(--accent-color); color: var(--accent-color);">
                                <?= icon('smartphone', 'icon-xs') ?> Open in UPI App
                            </a>
                        </div>
                ...
                <div style="background: var(--bg-main); border: 1px solid var(--border-color); border-radius: 8px; padding: 0.875rem; font-size: 0.8125rem; margin-bottom: 1.25rem; box-shadow: var(--shadow-xs);">
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.35rem;">
                        ...
                        <strong style="color: var(--text-primary);"><?= e($upiDetails['payee_name']) ?></strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.35rem;">
                ...
                <form method="POST" action="/fee/submit-utr">
                    ...
                    <button type="submit" class="btn-primary" style="width: 100%; padding: 0.75rem; font-weight: 800; font-size: 0.9375rem; display: inline-flex; align-items: center; justify-content: center; gap: 0.4rem;">
                        <?= icon('send', 'icon-xs') ?> Submit UTR for Counter Verification
                    </button>
                </form>
            </div>
        </div>
