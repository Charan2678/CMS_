<div style="margin-bottom: 1.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
    <div>
        <h1 style="font-size: 1.35rem; font-weight: 800; color: var(--text-primary); margin: 0;">
            Staff Profile: <?= e($staff['first_name'] . ' ' . $staff['last_name']) ?>
        </h1>
        <div style="font-size: 0.8125rem; color: var(--text-secondary); margin-top: 0.25rem;">
            <?= e($staff['employee_id']) ?> — <?= e($staff['designation_name']) ?>
        </div>
    </div>
    <a href="/staff" class="btn btn-secondary" style="font-size: 0.8125rem; display: inline-flex; align-items: center; gap: 0.35rem; text-decoration: none;">
...
<div style="display: grid; grid-template-columns: 1fr 3fr; gap: 1.5rem;">
    ...
    <div class="panel" style="text-align: center;">
        <div style="width: 96px; height: 96px; border-radius: 50%; background: linear-gradient(135deg, rgba(2, 132, 199, 0.16) 0%, rgba(2, 132, 199, 0.06) 100%); color: var(--accent-color); display: flex; align-items: center; justify-content: center; font-size: 2.5rem; margin: 0 auto 1rem auto; border: 3px solid var(--accent-color); box-shadow: var(--shadow-md);">
            <?= icon('briefcase', 'icon-xl') ?>
        </div>
        <h2 style="font-size: 1.25rem; font-weight: 800; color: var(--text-primary);"><?= e($staff['first_name'] . ' ' . $staff['last_name']) ?></h2>
        <p style="color: var(--text-secondary); font-size: 0.875rem; margin-bottom: 0.5rem; font-weight: 600;"><?= e($staff['employee_id']) ?></p>
        ...
        <div style="margin-top: 1.5rem; border-top: 1px solid var(--border-color); padding-top: 1rem; text-align: left; font-size: 0.8125rem;">
            ...
            <div style="margin-bottom: 0.75rem;">
                ...
                <?php if (!empty($staff['user_account_id'])): ?>
                    <span class="badge badge-success"><?= icon('check-circle-2', 'icon-xs') ?> Username: <?= e($staff['username']) ?></span>
                <?php else: ?>
    ...
    <div class="panel">
        <div class="panel-header">
            <h3 class="panel-title"><?= icon('user', 'icon-xs') ?> Non-Faculty Staff Contact Details</h3>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem; font-size: 0.875rem;">
            ...
            <div>
                ...
                <span class="badge badge-success" style="text-transform: capitalize;"><?= icon('check-circle-2', 'icon-xs') ?> <?= e($staff['status']) ?></span>
            </div>
        </div>
    </div>
</div>
