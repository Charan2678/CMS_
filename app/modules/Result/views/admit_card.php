...
<div style="max-width: 900px; margin: 0 auto;">
    ...
    <div class="no-print" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
        <div>
            <h1 style="font-size: 1.5rem; font-weight: 800; margin: 0; color: var(--text-primary); display: flex; align-items: center; gap: 0.5rem; letter-spacing: -0.015em;">
                <?= icon('ticket', 'icon-md') ?> Semester Examination Admit Card / Hall Ticket
            </h1>
        ...
        <?php if ($isEligible): ?>
            <button onclick="window.print()" class="btn-primary" style="display: inline-flex; align-items: center; gap: 0.5rem;">
                <?= icon('printer', 'icon-xs') ?> Print / Download Admit Card
            </button>
        <?php endif; ?>
    </div>
    ...
    <?php if (!$isEligible): ?>
        <!-- Blocked Alert Card -->
        <div class="panel" style="border-left: 4px solid var(--danger); background: rgba(239, 68, 68, 0.06); padding: 1.5rem; margin-bottom: 2rem;">
            <div style="display: flex; align-items: flex-start; gap: 1rem;">
                <div style="color: var(--danger); font-size: 1.5rem; margin-top: 0.15rem;">
                    <?= icon('ban', 'icon-lg') ?>
                </div>
                <div>
                    <h2 style="color: var(--danger); font-size: 1.25rem; font-weight: 800; margin: 0 0 0.5rem 0;">Admit Card Blocked — Exam Ineligible</h2>
                    ...
                    <ul style="margin: 0 0 1.25rem 1.25rem; padding: 0; color: var(--danger); font-weight: 600;">
                    ...
                    <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                        <?php if (($eligibility['pending_dues'] ?? 0) > 0): ?>
                            <a href="/fee/payments" class="btn-primary" style="font-size: 0.8125rem; display: inline-flex; align-items: center; gap: 0.35rem;"><?= icon('credit-card', 'icon-xs') ?> Clear Pending Dues Online</a>
                        <?php endif; ?>
                        <a href="/profile" class="btn btn-secondary btn-sm" style="display: inline-flex; align-items: center; gap: 0.35rem;"><?= icon('phone', 'icon-xs') ?> Contact Department HOD</a>
                    </div>
                </div>
            </div>
        </div>
    <?php else: ?>
        ...
        <div class="panel admit-card-container" style="background: #ffffff; color: #0f172a; border: 2px solid #0f172a; padding: 2rem; border-radius: 12px; box-shadow: var(--shadow-lg);">
            <!-- Header Seal & Title -->
            ...
            <div style="display: grid; grid-template-columns: 3fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem; background: #f8fafc; padding: 1.25rem; border-radius: 6px; border: 1px solid #e2e8f0;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.875rem;">
                        ...
                        <tr>
                            ...
                            <td style="padding: 4px 8px;">
                                <span style="color: #16a34a; font-weight: bold; display: inline-flex; align-items: center; gap: 0.25rem;">
                ...
                <div style="text-align: center;">
                    <div style="width: 105px; height: 125px; border: 2px solid #0f172a; margin: 0 auto; display: flex; align-items: center; justify-content: center; background: #e2e8f0; border-radius: 4px; overflow: hidden;">
                        ...
                        <?php else: ?>
                            <span style="font-size: 2rem; color: #64748b;"><?= icon('user', 'icon-xl') ?></span>
                        <?php endif; ?>
                    </div>
